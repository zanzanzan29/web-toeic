...
//	}
	...
	public String quanLyBaiNghePage() {
		...
		if(currentUser == null || currentUser.getVaiTro().getValue() != 2){
...
//		model.addAttribute("baitaptuvung", new BaiTapTuVung());
	...
	public String updateNguoiDung(@ModelAttribute NguoiDung nd, HttpServletRequest request) {
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		NguoiDung currentUsers = nguoiDungService.findByEmail(auth.getName());
		if(currentUsers == null || currentUsers.getVaiTro().getValue() != 2){
			return "redirect:/signin?accessDenied";
		}
